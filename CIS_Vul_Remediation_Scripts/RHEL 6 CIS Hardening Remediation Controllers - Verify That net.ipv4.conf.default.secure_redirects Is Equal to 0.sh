...
cat /etc/sysctl.conf | grep -v "^#" | grep -i net.ipv4.conf.default.secure_redirects &> /dev/null
...
if [[ "$?" == 1 ]];
	then
		...
		value=`cat /etc/sysctl.conf | grep -v "^#"| grep -i "net.ipv4.conf.default.secure_redirects" | awk -F= '{print $2}' | tr -d " "`  
		if [[ $value == 0 ]];
			then	
				/bin/echo "Settings are correct, no action required..Exting.."
			else
				/bin/echo "Changing the parameter value to 0"
				/bin/sed -i 's/net.ipv4.conf.default.secure_redirects = '"$value"'/net.ipv4.conf.default.secure_redirects = 0/g' /etc/sysctl.conf
